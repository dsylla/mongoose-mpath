...
mpathUtil.listToTree = (list, sort) => {
  ...
  for (index = 0; index < list.length; index += 1) {
    ...
    currentNode._doc.children = [];
    nodeMap[currentNode._id] = index;

    const hasParentInMap = nodeMap.hasOwnProperty(currentNode.parent);

    if (hasParentInMap) {
      list[nodeMap[currentNode.parent]]._doc.children.push(currentNode);

      if (shouldSort) {
        list[nodeMap[currentNode.parent]]._doc.children = _orderBy(
          list[nodeMap[currentNode.parent]]._doc.children,
          lodashSort.keys,
          lodashSort.orders
        );
      }
